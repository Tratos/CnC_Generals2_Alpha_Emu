...
namespace BlazeLibWV
{
    public static class BlazeGen
    {
        ...
        public static BlazeGen.TdfStruct CreateStructStub(List<Tdf> tdfs, bool has2 = false)
        {
            BlazeGen.TdfStruct res = new BlazeGen.TdfStruct();
            res.Values = tdfs;
            ...
            for (int i = 0; i < res.Count; i++)
            {
                switch (res.SubType)
                {
                    ...
                    case 3:
                        ...
                        BlazeGen.TdfStruct tmp = new BlazeGen.TdfStruct();
            ...
            for (int i = 0; i < res.Count; i++)
            {
                switch (res.SubType1)
                {
                    ...
                    case 3:
                        ...
                        BlazeGen.TdfStruct tmp = new TdfStruct();
                        tmp.startswith2 = false;
                        tmp.Values = ReadStruct(s, out tmp.startswith2);
                        l3.Add(tmp);
                        res.List1 = l3;
            ...
            for (int i = 0; i < tdf.Count; i++)
                switch (tdf.SubType)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 3:
                        BlazeGen.TdfStruct str = ((List<BlazeGen.TdfStruct>)tdf.List)[i];
                        if (str.startswith2)
                            s.WriteByte(2);
                        foreach (BlazeGen.Tdf ttdf in str.Values)
                            WriteTdf(ttdf, s);
                        s.WriteByte(0);
                        break;
                    case 9:
            ...
            for (int i = 0; i < tdf.Count; i++)
            {
                switch (tdf.SubType1)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 3:
                        BlazeGen.TdfStruct str = ((List<BlazeGen.TdfStruct>)tdf.List1)[i];
                        if (str.startswith2)
                            s.WriteByte(2);
                ...
                switch (tdf.SubType2)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 3:
                        BlazeGen.TdfStruct str = ((List<BlazeGen.TdfStruct>)tdf.List2)[i];
                        if (str.startswith2)
                            s.WriteByte(2);
                        foreach (Tdf ttdf in str.Values)
                            WriteTdf(ttdf, s);
